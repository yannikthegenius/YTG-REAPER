-- Basierend auf: Meo-Ada Mespotine 19th of February 2024 - licensed under MIT-license
-- Angepasst: Spielt einen Sound beim REAPER-Startup ab
...
-- Startup: Sound abspielen, falls einer konfiguriert ist
local soundfile = reaper.GetExtState(STATE_SECTION, STATE_KEY)

if soundfile ~= "" and reaper.file_exists(soundfile) then
  PreviewMediaFile(soundfile)
else
  -- Kein Sound ausgewählt – nachfragen, ob einer gewählt werden soll
  local answer = reaper.MB("No sound yet selected, do you want to choose one?", "No sound yet", 4)
  if answer == 6 then
    local retval, filename = reaper.GetUserFileNameForRead("", "Select Startup Sound", "*.mp3;*.wav;*.flac;*.mp4;*.aif")
    if retval == true then
